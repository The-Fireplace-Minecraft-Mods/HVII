            ...
            switch(button.id){
                ...
                default:
                    if(button instanceof ToggleButton){
                        ...
                        if(flag){
                            tmpMobs = ArrayUtils.remove(tmpMobs, ArrayUtils.indexOf(tmpMobs, currentlyShownMobs[button.id-2].getNamespace()));
                        }else{
                            tmpMobs = ArrayUtils.add(tmpMobs, currentlyShownMobs[button.id-2].getNamespace());
                        }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int x=0;x<8;x++){
            if(currentlyShownMobs[x] != null)
                drawString(fontRenderer, currentlyShownMobs[x].getNamespace(), guiLeft+24, guiTop+10+x*18, Color.WHITE.getRGB());
        }
    }
        ...
        if ((page == max_page) && (length != ((max_page - 1) * 8))) {
            ...
            for (int i = 2; i < 10; i++) {
                ...
                if (tmp < length) {
                    ...
                    ((ToggleButton)buttonList.get(i)).toggled= ArrayUtils.contains(tmpMobs, currentlyShownMobs[i-2].getNamespace());
                } else {
